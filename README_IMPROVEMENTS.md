...
- 工作方式：按符号逐一处理，每个符号到达时即在相关器前进行多普勒扫描，无需等待帧完成
...
| 改进 | 相对计算量 | 关键操作 | 实时可行性 | 原因 |
|------|----------|--------|----------|------|
| 多普勒补偿 | ×5~10 | 多普勒扫描循环 | ✓ 可行 | 按符号处理，无帧延迟 |
| LMS均衡 | ×2~3 | LMS权重更新 | ✓ 可行 | 在线权重更新，流式结构 |
| Viterbi解码 | ×50~100 | 状态转移计算 | ✗ 不可行 | 需回溯积累 ≈5K 符号，产生帧级延迟 |
| **多普勒+LMS** | **×7~13** | 两者组合 | ✓ 可行 | 合计延迟 < 100ms，硬件可承受 |
| **全部改进** | **×100~200** | 上述组合 | ✗ 不可行 | Viterbi 回溯延迟是主要瓶颈 |

**建议**：
- **实时系统**：仅用多普勒补偿 + LMS（计算量 ×7~13，可用于低功耗嵌入式硬件）
- **离线处理**：可启用全部改进以获最佳性能（编码增益 3-4 dB）
...
**A**: 
- ✓ 多普勒补偿 + LMS 可用于实时系统
- ✗ Viterbi 解码不可用于实时系统，原因如下：
  - **回溯延迟**：Viterbi 算法需要向后回溯 ≈5K 个符号才能确定当前输出，导致帧级延迟（对应码长 200 比特时延迟约 5-20 ms）
  - **计算量**：×50~100 的相对计算量在低功耗硬件上无法满足符号速率约束
  - **流式处理不兼容**：需要完整帧数据才能完成解码，违反实时按符号处理的要求
- 仅多普勒+LMS 的预计延迟：10-100 ms（取决于比特数和采样率）
- 部署建议：需优化 MATLAB 代码为 C/C++ 并在嵌入式硬件上验证
